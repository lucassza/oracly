<x-filament-panels::page>
    ...
    <x-oracly.page-header eyebrow="Operação diária · estratégias contra">
        ...
        <x-slot name="description">
            @if ($mode === 'upcoming')
                {{ \Carbon\Carbon::parse($date)->format('d/m/Y') }} · uma linha para cada estratégia disponível.
            @else
                Histórico consolidado das entradas LAY selecionadas pelo mesmo ranking da operação diária.
            @endif
        </x-slot>
    ...
    @if ($mode === 'upcoming')
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div
                wire:key="lay-copy-{{ $mode }}-{{ $date }}-{{ $hourFilter }}"
                x-data="{
                    copied: false,
                    rows: @js($this->filteredRows),
                    copyAll() {
                        const lines = [];
                        this.rows.forEach((row) => {
                            const time = new Date(row.kickoffAt).toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' });
                            row.bets.forEach((bet) => {
                                lines.push(`${time}\t${row.homeTeam} x ${row.awayTeam}\t${bet.bet}`);
                            });
                        });
    ...
    @else
        <div class="grid gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-2 xl:grid-cols-4 dark:border-white/10 dark:bg-white/[0.04]">
            ...
            <label class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                Estratégia
                <select wire:model.live="historyStrategyFilter" class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500 dark:border-white/10 dark:bg-white/5 dark:text-white">
                    <option value="all">Todas</option>
                    @foreach ($this->historyStrategies as $strategy)
        ...
        @if (count($this->historyStrategyStats))
            <section>
                <div class="mb-3">
                    <h2 class="text-sm font-semibold text-gray-950 dark:text-white">Assertividade por estratégia</h2>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Resultados das estratégias conforme os filtros selecionados.</p>
                </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
                <div class="mb-3 flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">Últimos {{ count($this->historyChartDays) }} dias com entradas LAY liquidadas.</p>
                    </div>
    ...
    <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
        <table class="w-full text-left text-sm">
            <thead class="bg-gray-50 text-xs font-semibold tracking-wide text-gray-500 uppercase dark:bg-white/[0.03] dark:text-gray-400">
                <tr>
                    <th class="w-24 px-4 py-2">{{ $mode === 'history' ? 'Data' : 'Hora' }}</th>
                    <th class="px-4 py-2">Partida</th>
                    <th class="w-64 px-4 py-2">Estratégias</th>
                </tr>
            </thead>
                            ...
                            <div x-data="{ copied: '', copy(text, team) { navigator.clipboard.writeText(text).then(() => { this.copied = team; setTimeout(() => this.copied = '', 2000) }) } }" class="space-y-1">
                                ...
                                @if ($row['competition'])
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $row['country'] ? $row['country'].' · ' : '' }}{{ $row['competition'] }}
                                    </div>
                                @endif
                                @if ($mode === 'upcoming' && $row['status'] === 'finished' && $row['homeScore'] !== null && $row['awayScore'] !== null)
                                    <div class="flex flex-wrap items-center gap-2 text-xs">
                                        <span class="rounded-full bg-emerald-100 px-2 py-1 font-semibold text-emerald-800 dark:bg-emerald-400/15 dark:text-emerald-200">Finalizado</span>
                                        <span class="font-semibold text-gray-700 dark:text-gray-200">FT {{ $row['homeScore'] }}-{{ $row['awayScore'] }}</span>
                                    </div>
                                @endif
                                @if ($mode === 'history')
                                    <div class="flex flex-wrap items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                                        <span>{{ $row['halftimeHomeScore'] !== null && $row['halftimeAwayScore'] !== null ? 'HT '.$row['halftimeHomeScore'].'-'.$row['halftimeAwayScore'] : 'HT —' }}</span>
                                        <span>{{ $row['homeScore'] !== null && $row['awayScore'] !== null ? 'FT '.$row['homeScore'].'-'.$row['awayScore'] : 'FT —' }}</span>
                                    </div>
                                @endif
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                            {{ $mode === 'history' ? 'Nenhuma entrada histórica LAY para os filtros atuais.' : 'Nenhuma estratégia LAY para esta data.' }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if ($mode === 'history' && $this->historyPagination['total'] > 0)
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Exibindo {{ $this->historyPagination['from'] }}–{{ $this->historyPagination['to'] }} de {{ $this->historyPagination['total'] }} jogos
            </p>
